...
COMMAND_LINE *getInputParameter(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
        UINTN OutPutBufSize=100;
        CHAR16*OutPutStr;
        ...
        Status = pBS->AllocatePool (EfiBootServicesData,OutPutBufSize,&OutPutStr);//need judge!
          memset(OutPutStr,0x00,OutPutBufSize);  
          
	pBS->OpenProtocol(ImageHandle,&gEfiShellParametersProtocolGuid,(VOID **)&EfiShellParametersProtocol,
	                               ImageHandle,NULL,EFI_OPEN_PROTOCOL_GET_PROTOCOL);
	  if(EfiShellParametersProtocol!=NULL)
	  {
	          //Swprintf(OutPutStr+wcslen(OutPutStr),L"EfiShellParametersProtocol OK. par cnt= %d.\n\r",EfiShellParametersProtocol->Argc);
		  gST->ConOut->OutputString (gST->ConOut,OutPutStr);
		  memset(OutPutStr,0x00,OutPutBufSize);
		  Argc=EfiShellParametersProtocol->Argc;
		  Argv=EfiShellParametersProtocol->Argv;
		  //pBS->FreePool(OutPutStr);
		  //return EFI_SUCCESS;
	  }
	  else
	 {
	   pBS->OpenProtocol(ImageHandle,&gEfiShellInterfaceGuid,(VOID **)&EfiShellInterface,
			  ImageHandle,NULL,EFI_OPEN_PROTOCOL_GET_PROTOCOL);
	   if(EfiShellInterface!=NULL)
	   {
	          //Swprintf(OutPutStr+wcslen(OutPutStr),L"EfiShellInterface OK. par cnt= %d.\n\r",EfiShellInterface->Argc);
	 	  gST->ConOut->OutputString (gST->ConOut,OutPutStr);
		  memset(OutPutStr,0x00,OutPutBufSize);
	   ...
	   if((EfiShellParametersProtocol==NULL)&&(EfiShellInterface==NULL))
	   {
	          Swprintf(OutPutStr+wcslen(OutPutStr),L"every shell interface is not OK.\n\r");
	 	  gST->ConOut->OutputString (gST->ConOut,OutPutStr);
	 	  pBS->FreePool(OutPutStr);
	 	  return NULL;
	   }
...
void GetInputString(CHAR16 *str, UINTN len)
{
	EFI_SIMPLE_TEXT_INPUT_PROTOCOL *ConIn;//*Simple_In,
	EFI_STATUS Status;
	EFI_INPUT_KEY Key;
	UINTN EventIndex;
	UINTN index=0;
	...
	while(len--)
	{
		Status = pBS->WaitForEvent (1, &(ConIn->WaitForKey), &EventIndex);
		if (Status != EFI_SUCCESS)
		    continue;
		Status = ConIn->ReadKeyStroke(ConIn,&Key);
		if (Status != EFI_SUCCESS)
		    continue;
		tmp[index++]=Key.UnicodeChar;
...
void StrLwr(CHAR16 *src)
{
	UINTN len=StrLen(src),i;
	for(i=0;i<len;i++)
	{
...
UINT16 GPNV_Initial(UINT32 FADR)
{
	...
	Print(L"FSTAPtr %x\r\n",FSTAPtr);
	Print(L"FSFNPtr %x\r\n",FSFNPtr);
	Print(L"BIOS_FlashSMI->SMIPort %x\r\n",BIOS_FlashSMI->SMIPort);
	Print(L"BIOS_FlashSMI->FlashSwSmiCode %x\r\n",BIOS_FlashSMI->FlashSwSmiCode);
#endif
...
{
	UINT32 EXTFPtr, FlashSMIPtr, actual_flashsize, ROMStart;
#ifdef CDEBUG_INFO
	UINT32 BIOSInfoPtr;
#endif
	UINT16 status;
	...
	//Get SMI Information 
	FlashSMIPtr = (0xF0000 + BIOS_EXTF->FlashSmiTable);
...
UINT16 MultiSetGPNV(UINT32 sig, UINT32 addr, CHAR16 *data)
{
	...
	for (i=0; i<wSize; i++)
	{
		...
		if (wDataSize%2 && i==0)
		{
			SSPrint(HexStr, sizeof(HexStr), L"%02c", *(data+i));
			wData[i] = (UINT8)StrHexToUintn(HexStr);
			odd = 1;
		}
		else
		{
			SSPrint(HexStr, sizeof(HexStr), L"%c%c", *(data+i*2-odd), *(data+i*2+1-odd));
			wData[i] = (UINT8)StrHexToUintn(HexStr);
		}
	}
	...
	for (i=0; i<size; i++)
	{
		...
		if (i!=0 && (i+1)%16==0)
		{
			...
			{				
				...
				{
					SetMem(HexStr, sizeof(HexStr), 0);
					SSPrint(HexStr, sizeof(HexStr), L"%c", GPNVBuffer);
					Print(HexStr);
				}
				c++;
			}
...
//				gST->ConOut->EnableCursor(gST->ConOut, 0);
				//Input(NULL, HexStr, 1);		//clare
				GetInputString(HexStr,1);
...
//DRK_UBT008- 			parameter = parameter & ~CMD_BLK;
	...
	if ((parameter&CMD_WRITE) != 0)
	{
		...
		if (StrHexToUintn(SI->Argv[4])>=4&&!StrCmp(SI->Argv[2], L"ASFG") ||
		    (StrHexToUintn(SI->Argv[4])+size-1)>=4&&!StrCmp(SI->Argv[2], L"ASFG"))
			parameter = parameter & ~CMD_WRITE;
		else if (StrHexToUintn(SI->Argv[4])>0xff && StrCmp(SI->Argv[2], L"_DMI"))
			parameter = parameter & ~CMD_WRITE;
		else if (StrHexToUintn(SI->Argv[4])> 0x7ff)
			parameter = parameter & ~CMD_WRITE;
	}
	...
	if ((parameter&CMD_WFT) != 0)
	{
		...
		if (StrHexToUintn(SI->Argv[4]) == 0)
			parameter = parameter & ~FE;
				
		if (!IsNumber(SI->Argv[6]))
			parameter = parameter & ~FE;

		if ((StrHexToUintn(SI->Argv[4])+StrHexToUintn(SI->Argv[6])-1)>=4 && !StrCmp(SI->Argv[2], L"ASFG"))
			parameter = parameter & ~FE;
		else if ((StrHexToUintn(SI->Argv[4])+StrHexToUintn(SI->Argv[6])-1)>0xff && StrCmp(SI->Argv[2], L"_DMI"))
			parameter = parameter & ~FE;
		else if ((StrHexToUintn(SI->Argv[4])+StrHexToUintn(SI->Argv[6])-1) > 0x7ff)
			parameter = parameter & ~FE;	
	}
...
//	EFI_NO_BOOTSHELL_CODE(EFI_SHELL_APP_INIT (ImageHandle, SystemTable));
	...
	SI=getInputParameter(ImageHandle,SystemTable);
#ifdef CDEBUG_INFO
	Print(L"argc : %d\r\n",SI->Argc);
	for(argc_index=0;argc_index<SI->Argc;argc_index++)
	{
		Print(L"argv %d : %s\r\n",argc_index,SI->Argv[argc_index]);
	}
#endif
...
//DRK_UBT008- 		dataSize = GetGPNV(sig);
		...
		MultiSetGPNV(sig, (UINT32)StrHexToUintn(SI->Argv[4]), SI->Argv[5]);
	}
	else if (CheckOperation(operation, WRITE_TXT_OPERATION))
	{
//DRK_UBT008- 		dataSize = GetGPNV(sig);
		ShowTitle(dataSize, sig);
		RWGPNVFile(ImageHandle, sig, (UINT32)StrHexToUintn(SI->Argv[6]), (UINT16)StrHexToUintn(SI->Argv[4]), SI->Argv[8], 1, 1);
	}
	else if (CheckOperation(operation, WRITE_BIN_OPERATION))
	{
//DRK_UBT008- 		dataSize = GetGPNV(sig);
		ShowTitle(dataSize, sig);
		RWGPNVFile(ImageHandle, sig, (UINT32)StrHexToUintn(SI->Argv[6]), (UINT16)StrHexToUintn(SI->Argv[4]), SI->Argv[8], 0, 1);
	}
	else if (CheckOperation(operation, READ_TXT_OPERATION))
	{
//DRK_UBT008- 		dataSize = GetGPNV(sig);
		ShowTitle(dataSize, sig);
		RWGPNVFile(ImageHandle, sig, (UINT32)StrHexToUintn(SI->Argv[6]), (UINT16)StrHexToUintn(SI->Argv[4]), SI->Argv[8], 1, 0);
	}
	else if (CheckOperation(operation, READ_BIN_OPERATION))
	{
//DRK_UBT008- 		dataSize = GetGPNV(sig);
		ShowTitle(dataSize, sig);
		RWGPNVFile(ImageHandle, sig, (UINT32)StrHexToUintn(SI->Argv[6]), (UINT16)StrHexToUintn(SI->Argv[4]), SI->Argv[8], 0, 0);
	}
	else if (CheckOperation(operation, RMAC_OPERATION))
	{
//DRK_UBT008- 		dataSize = GetGPNV(sig);
		ShowTitle(dataSize, sig);
		GetMAC(sig, (UINT32)StrHexToUintn(SI->Argv[4]));
	}
